...
// $this->performAjaxValidation($model);
    ...
    private function handleAjaxRequest() {
        ...
        $result = array();
        $action = $_GET['action'];
        if ($action == 'createCarVersion') {
            $carVersion = new CarVersion();
            if (isset($_POST['CarVersion'])) {
                $carVersion->attributes = $_POST['CarVersion'];
                if ($carVersion->validate() && $carVersion->save()) {
                    $result['success'] = true;
                    $result['carVersionId'] = $carVersion->id;
                } else {
                    $result['success'] = false;
                    $result['errors'] = $carVersion->errors;
                }
            }
        } else if ($action == 'getCarOptions') {
            $result['carOptions'] = array();
            $carOptions = CarOption::model()->findAll();
            foreach ($carOptions as $carOption) {
                $result['carOptions'][] = $carOption->attributes;
            }
            $result['success'] = true;
        }
